...
module.exports.init = function (app, opts) {
  ...
  client.on('ready', function () {
    ...
    if (web.multiTenancy.enabled) {
      ...
      // I think this logic should me moved into a configuration strategy in this library
      ...
      if (web.multiTenancy.organizationResolver) {
        ...
        //
        // Do we?  we're calling addPostRoute() for the registration handler below, cant we just
        // move this block below that one?
      ...
      // enabling globally will cause all POSTs to query for organization context?
      // if so we need to limit this to just the POST routes that we handle
