    ...
    forms.loginForm.handle(req, {
      ...
      success: function (form) {
        application.getAccountStoreMappings({
          ...
          /*
            This will read all of the account store mappings for the application.
            If there are many (hundreds), this may take a few seconds to complete
            because the API has to be paginated.
           */
          collection.detect(function (mapping, next) {
            next(mapping.accountStore.name === form.data.directoryName);
          }, function (accountStoreMapping) {

            if (accountStoreMapping) {
              authenticator.authenticate({
                username: form.data.login,
                password: form.data.password,
                accountStore: accountStoreMapping.accountStore.href
              }, function (err, passwordGrantAuthenticationResult) {
                if (err) {
              ...
              return renderForm(form, { error: 'Invalid organization name.' });
            }
          });
        });
      },
