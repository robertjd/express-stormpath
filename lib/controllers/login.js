    ...
    forms.loginForm.handle(req, {
      ...
      success: function (form) {
        ...
        client.getOrganizations({
          nameKey: form.data.organizationNameKey
        }, function (err, collection) {

          if (collection.items.length === 1 && collection.items[0].nameKey === form.data.organizationNameKey) {
            authenticator.authenticate({
              username: form.data.login,
              password: form.data.password,
              accountStore: collection.items[0].href
            }, function (err, passwordGrantAuthenticationResult) {
              if (err) {
