...
/**
 * Given the posted email address, attempt to locate the user. If found, reset the
 * token and call the email verification handler.
 *
 * As before, we don't error if the account cannot be found, so this callback will
 * always be called with no parameters.
 *
 * @param {*} Express Request
 * @param {*} callback
 */
function resetToken(req, callback) {
  var client = req.app.get('stormpathClient');
  var config = req.app.get('stormpathConfig');
  var emailVerificationHandler = config.emailVerificationHandler;
  var logger = req.app.get('stormpathLogger');

  client.getAccounts({ q: req.body.email}, function (err, accounts) {
    if (err) {
      logger.error(err);
    }
