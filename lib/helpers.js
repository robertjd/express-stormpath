...
module.exports.getUser = function(req, res, next) {
  ...
  if (req.session && req.session.user) {
    ...
    req.app.get('stormpathClient').getAccount(req.session.user.href, options, function(err, account) {
      if (err) {
        // if verbose option, console.error the error
        req.session.destroy();
        return next();
      }
  ...
  // authentication.
    ...
    req.app.get('stormpathApplication').authenticateApiRequest({ request: req }, function(err, authResult) {
      if (err) {
        // if not a custom api auth error handler, then just res.json(err.statusCode,err)
        // else! call the custom handler with (err)
      } else {
